...
- 다중 Node 버전, MCP smoke, 보안 스캔, 설치 캐시 일치 검증 수행
...
6. **제3자 패키지**: 재배포 전 각 패키지의 실제 버전과 라이선스 고지를 다시 검사합니다. 프로젝트 라이선스가 제3자 구성요소의 조건을 덮어쓰지 않습니다.
...
- 핵심 검증 236개 통과
- MCP smoke 검증 18개 통과
- Node.js 18, 20, 26에서 검증 통과
- Codex 포팅 검증 115개 통과
- 공식 플러그인 validator의 소스·설치 캐시 검증 통과
- JavaScript 모듈 30개 구문 검사, JSON 9개 파싱, YAML 파싱 통과
- 보안 검사 88개 통과, npm production/full audit 취약점 0
- 설치 캐시 파일 44개 해시 일치 및 플러그인 활성화 확인
- 등록된 볼트 읽기 전용 검사: 11개 존재, 2개 열림, 이름 충돌 0
- 5,000노트 기준 내부 benchmark: 약 2.385초, heap 증가 약 2.68 MiB
...
- 원격 GitHub Actions 결과는 저장소 공개와 PR 생성 후 별도로 확인해야 합니다.
- 현재 Git 작업 트리에 많은 포팅 변경이 있으며 원격은 원본 저장소만 연결되어 있으므로 그대로 공개 push하면 안 됩니다.
